...
type GetEventListenersParams struct {
	...
	Pierce   bool                   `json:"pierce,omitempty"` // Whether or not .0iframes and shadow roots should be traversed when returning the subtree (default is false). Reports listeners for all contexts if pierce is enabled.
}
