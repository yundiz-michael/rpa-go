...
func TestPrematureCancel(t *testing.T) {
	...
	if err := Run(ctx, nil); err != context.Canceled {
...
func TestPrematureCancelTab(t *testing.T) {
	...
	// tab.
	cancel()
	if err := Run(ctx2, nil); err != context.Canceled {
		t.Fatalf("wanted canceled context error, got %v", err)
	}
}
...
func TestLargeEventCount(t *testing.T) {
	...
	if err := Run(ctx,
		...
		WaitVisible("#done", nil, ByID), // wait for the JS to finish
	); err != nil {
		t.Fatal(err)
	}
}
