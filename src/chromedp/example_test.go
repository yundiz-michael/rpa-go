...
func ExampleExecAllocator() {
	...
	if err := chromedp.Run(taskCtx, nil); err != nil {
		log.Fatal(err)
	}
...
func ExampleNewContext_manyTabs() {
	...
	if err := chromedp.Run(ctx1, nil); err != nil {
		log.Fatal(err)
	}
	...
	if err := chromedp.Run(ctx2, nil); err != nil {
		log.Fatal(err)
	}
...
func ExampleEmulate() {
	...
	if err := chromedp.Run(ctx,
		...
		chromedp.WaitVisible(`#zci-answer`, nil, chromedp.ByID),
...
func ExamplePrintToPDF() {
	...
	var buf []byte
	if err := chromedp.Run(ctx,
		chromedp.Navigate(`https://pkg.go.dev/merkaba/chromedp/chromedp`),
...
<body>
  <div id="content">the content</div>
	...
	if err := chromedp.Run(ctx,
		...
		chromedp.WaitVisible(`#content`, nil, nil),
		chromedp.ActionFunc(func(ctx context.Context) error {
		...
		chromedp.WaitVisible(`#thing`, nil, nil),
	); err != nil {
		log.Fatal(err)
	}
