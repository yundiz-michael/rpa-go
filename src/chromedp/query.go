...
type Selector struct {
	query         interface{}
	createdTime   int64
	ActionTimeout int64 /*每个action的最长时间阈值*/
	MaxWaitTime   int64 /*增加过期时间，避免by发现为空后，一直循环等待*/
	fromNode      *cdp.Node
	WebPage       any
...
/*增加一个查询的参数,timeout所有*/
func Query(sel interface{}, param interface{}, opts ...QueryOption) QueryAction {
	s := &Selector{
		query:         sel,
		exp:           1,
		createdTime:   time.Now().UnixMilli(),
		ActionTimeout: 0,
		MaxWaitTime:   int64(20000),
	}
	// apply options
	for _, o := range opts {
		o(s, param)
...
func (s *Selector) Do(ctx context.Context) error {
	...
	/*retryWithSleep用了一个for的无限循环*/
	return retryWithSleep(ctx, 100*time.Millisecond, func(ctx context.Context) (bool, error) {
		...
		if err != nil || len(ids) < s.exp {
			t := time.Now().UnixMilli() - fromTime
			/*每个sel如果超过过期时间还没有出现，且没有注册处理器，操作将全部结束，否则以处理器返回结果为准*/
			if s.MaxWaitTime > 0 && t >= s.MaxWaitTime*1000 {
				fromTime = time.Now().UnixMilli()
				if s.ScriptHandler != nil {
					ok, err = s.ScriptHandler.HandleTimeout(s.WebPage, err)
					return ok, err
				} else {
					return true, ErrorTimeoutHandler
				}
			} else {
				if actionTimeout {
					return true, ErrActionTimeout
				} else {
					return false, nil
				}
			}
...
// selAsString forces query into a string.
func (s *Selector) selAsString() string {
	if sel, ok := s.query.(string); ok {
		return sel
	}
	return fmt.Sprintf("%s", s.query)
}

func (s *Selector) NodesMoreThan(min int) func(context.Context, *cdp.Frame, runtime.ExecutionContextID, ...cdp.NodeID) ([]*cdp.Node, error) {
...
// WaitReady waits for the specified nodes to be ready.返回一个函数指针
...
// query. Waits until the visibility conditions of the query have been met, after
// which executes f.
func QueryAfter(sel interface{}, param interface{}, f func(context.Context, runtime.ExecutionContextID, ...*cdp.Node) error, opts ...QueryOption) QueryAction {
	return Query(sel, param, append(opts, After(f))...)
}

// QueryOption is an element query action option.增加一个动态参数
type QueryOption = func(selector *Selector, param interface{})
...
func FromNode(node *cdp.Node) QueryOption {
	return func(s *Selector, param interface{}) { s.fromNode = node }
}

func ActionTimeout(second int64) QueryOption {
	return func(s *Selector, param interface{}) { s.ActionTimeout = second * 1000 }
}

func SetRunParam(scriptHandler common.ScriptHandler, webPage any, maxWaitTime int64) QueryOption {
	return func(s *Selector, param interface{}) {
		s.WebPage = webPage
		s.ScriptHandler = scriptHandler
		s.MaxWaitTime = maxWaitTime
	}
}

func SetScrollIntoView() QueryOption {
...
func ByQuery(s *Selector, param interface{}) {
	ByFunc(func(ctx context.Context, n *cdp.Node) ([]cdp.NodeID, error) {
		...
		if nodeID == cdp.EmptyNodeID {
			return []cdp.NodeID{}, ErrorNodeEmpty
		}
...
func ByQueryAll(s *Selector, param interface{}) {
	ByFunc(func(ctx context.Context, n *cdp.Node) ([]cdp.NodeID, error) {
...
func ByID(s *Selector, param interface{}) {
	s.query = "#" + strings.TrimPrefix(s.selAsString(), "#")
	ByQuery(s, param)
}
...
func BySearch(s *Selector, param interface{}) {
	ByFunc(func(ctx context.Context, n *cdp.Node) ([]cdp.NodeID, error) {
...
func ByJSPath(s *Selector, param interface{}) {
	ByFunc(func(ctx context.Context, n *cdp.Node) ([]cdp.NodeID, error) {
		// set up eval command
...
func ByNodeID(s *Selector, param interface{}) {
	ids, ok := s.query.([]cdp.NodeID)
	if !ok {
...
func NodeReady(s *Selector, param interface{}) {
	WaitFunc(s.WaitReady(nil))(s, param)
}

func NodeCountMoreThan(s *Selector, param interface{}) {
	WaitFunc(s.NodesMoreThan(param.(int)))(s, param)
}
...
func NodeVisible(s *Selector, param interface{}) {
	//waitReady里面的函数，是检查函数，再waitReady函数内部检查是否通过
...
func NodeNotVisible(s *Selector, param interface{}) {
	WaitFunc(s.WaitReady(func(ctx context.Context, execCtx runtime.ExecutionContextID, n *cdp.Node) error {
		// check box model
		_, err := dom.GetBoxModel().WithNodeID(n.NodeID).Do(ctx)
		if err != nil {
...
// 'disabled' attribute).
func NodeEnabled(s *Selector, param interface{}) {
	WaitFunc(s.WaitReady(func(ctx context.Context, execCtx runtime.ExecutionContextID, n *cdp.Node) error {
		n.RLock()
		defer n.RUnlock()
...
// attribute).
func NodeSelected(s *Selector, param interface{}) {
	WaitFunc(s.WaitReady(func(ctx context.Context, execCtx runtime.ExecutionContextID, n *cdp.Node) error {
		n.RLock()
		defer n.RUnlock()
...
func NodeNotPresent(s *Selector, param interface{}) {
	s.exp = 0
...
func WaitVisible(sel interface{}, opts ...QueryOption) QueryAction {
	return Query(sel, nil, append(opts, NodeVisible)...)
}

func WaitMoreThan(sel interface{}, count int, opts ...QueryOption) QueryAction {
	return Query(sel, count, append(opts, NodeCountMoreThan)...)
}

// WaitNotVisible is an element query action that waits until the element
// matching the selector is not visible.
func WaitNotVisible(sel interface{}, param interface{}, opts ...QueryOption) QueryAction {
	return Query(sel, param, append(opts, NodeNotVisible)...)
}
...
func WaitEnabled(sel interface{}, param interface{}, opts ...QueryOption) QueryAction {
	return Query(sel, param, append(opts, NodeEnabled)...)
}
...
func WaitSelected(sel interface{}, param interface{}, opts ...QueryOption) QueryAction {
	return Query(sel, param, append(opts, NodeSelected)...)
}

// WaitNotPresent is an element query action that waits until no elements are
// present matching the selector.
func WaitNotPresent(sel interface{}, param interface{}, opts ...QueryOption) QueryAction {
	return Query(sel, param, append(opts, NodeNotPresent)...)
}
...
func SetJavascriptAttribute(sel interface{}, name, value string, opts ...QueryOption) QueryAction {
	return QueryAfter(sel, value, func(ctx context.Context, execCtx runtime.ExecutionContextID, nodes ...*cdp.Node) error {
		if len(nodes) < 1 {
			return fmt.Errorf("selector %q did not return any nodes", sel)
		}
...
// https://merkaba/chromedp/examples/tree/master/keys.
//
...
func SendElementKeys(n *cdp.Node, v string) Action {
...
func SetElementJavascriptAttribute(n *cdp.Node, name, value any) Action {
	return ActionFunc(func(ctx context.Context) error {
		var res string
		err := CallFunctionOnNode(ctx, n, setAttributeJS, &res, name, value)
		if err != nil {
			return err
		}
		return nil
