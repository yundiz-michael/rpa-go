...
func (r *Runtime) builtinJSON_stringify(call FunctionCall) Value {
	...
	if ctx.do(call.Argument(0)) {
		content := ctx.normalizeContent(ctx.buf.String())
		if ctx.allAscii {
			return asciiString(content)
		} else {
...
func (ctx *_builtinJSON_stringifyContext) normalizeContent(content string) string {
	...
	for {
		...
		default:
			if r < 0x20 {
				...
				ctx.buf.WriteByte(hex_[r>>4])
				ctx.buf.WriteByte(hex_[r&0xF])
			} else {
				if utf16.IsSurrogate(r) {
					ctx.buf.WriteString(`\u`)
					ctx.buf.WriteByte(hex_[r>>12])
					ctx.buf.WriteByte(hex_[(r>>8)&0xF])
					ctx.buf.WriteByte(hex_[(r>>4)&0xF])
					ctx.buf.WriteByte(hex_[r&0xF])
				} else {
					ctx.buf.WriteRune(r)
