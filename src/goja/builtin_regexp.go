...
func writeHex4(b *strings.Builder, i int) {
	b.WriteByte(hex_[i>>12])
	b.WriteByte(hex_[(i>>8)&0xF])
	b.WriteByte(hex_[(i>>4)&0xF])
	b.WriteByte(hex_[i&0xF])
}
...
L:
	for {
		...
		case '/', '\u000a', '\u000d', '\u2028', '\u2029':
			...
			default:
				sb.WriteRune('u')
				sb.WriteRune(rune(hex_[c>>12]))
				sb.WriteRune(rune(hex_[(c>>8)&0xF]))
				sb.WriteRune(rune(hex_[(c>>4)&0xF]))
				sb.WriteRune(rune(hex_[c&0xF]))
			}
			lastPos = pos + size
		}
		pos += size
