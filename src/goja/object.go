...
const (
	...
	classWebClient   = "WebClient"
	classWebPage     = "WebPage"
	classWebResponse = "WebResponse"
	classWebElement  = "WebElement"
...
func (o *baseObject) getStr(name unistring.String, receiver Value) Value {
	...
	if prop == nil {
		if o.prototype != nil {
			self := o.prototype.self
			/*修复bug*/
			if self == o {
				return nil
			}
			if receiver == nil {
				return self.getStr(name, o.val)
			}
			return self.getStr(name, receiver)
		}
	}
	if prop, ok := prop.(*valueProperty); ok {
		if receiver == nil {
